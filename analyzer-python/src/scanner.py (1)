...
Qualification metrics:
  ...
  - Exact Aave V3 LiquidationLogic sizing via aave_v3_liquidation_math:
      * close-factor (50% of total debt vs 100% when HF <= 0.95 or position < $2k)
      * dust / leftover prevention (MustNotLeaveDust)
      * protocol fee taken from the liquidation bonus
...
# Exact pure-math port of Aave V3 LiquidationLogic
from aave_v3_liquidation_math import (
    LiquidationParams,
    compute_liquidation_amounts,
    MIN_BASE_MAX_CLOSE_FACTOR_THRESHOLD as MIN_BASE_THRESHOLD_INT,
)
...
# Aave V3 constants (display / legacy helpers; exact math lives in aave_v3_liquidation_math)
CLOSE_FACTOR_HF_THRESHOLD = Decimal("0.95")
DEFAULT_CLOSE_FACTOR = Decimal("0.5")
MAX_CLOSE_FACTOR = Decimal("1.0")
MIN_BASE_MAX_CLOSE_FACTOR_THRESHOLD = Decimal(MIN_BASE_THRESHOLD_INT)
    ...
    async def scan_users(self, users: List[str]) -> List[Opportunity]:
        ...
        for user in users:
            try:
                ...
                # Exact Aave V3 LiquidationLogic sizing (close-factor, dust, protocol fee)
                try:
                    debt_price = int(
                        await self.oracle.functions.getAssetPrice(debt_asset).call()
                    )
                    coll_price = int(
                        await self.oracle.functions.getAssetPrice(best_coll).call()
                    )
                    if coll_price == 0 or debt_price == 0:
                        continue

                    # healthFactor from getUserAccountData is in ray (1e18)
                    hf_ray = int(hf * Decimal(10**18))

                    liq_params = LiquidationParams(
                        health_factor=hf_ray,
                        total_debt_in_base=debt_base,
                        borrower_reserve_debt=debt_amount,
                        borrower_collateral_balance=colls[best_coll],
                        debt_asset_price=debt_price,
                        debt_asset_unit=10 ** debt_params.decimals,
                        collateral_asset_price=coll_price,
                        collateral_asset_unit=10 ** best_params.decimals,
                        liquidation_bonus=best_bonus_bps,
                        liquidation_protocol_fee_pct=best_lpf,
                        requested_debt_to_cover=debt_amount,  # ask for max; math applies close-factor
                    )
                    coll_amount, debt_to_cover, _protocol_fee, is_valid = (
                        compute_liquidation_amounts(liq_params)
                    )
                    if not is_valid or debt_to_cover <= 0 or coll_amount <= 0:
                        continue

                    # close_factor for logging / Opportunity payload
                    close_factor = (
                        Decimal(debt_to_cover) / Decimal(debt_amount)
                        if debt_amount > 0
                        else DEFAULT_CLOSE_FACTOR
                    )
                except Exception as e:
                    print(f"[warn] exact sizing failed for {user}: {e}", file=sys.stderr)
                    continue
